...
class SectorCobro:
    ...
    def __init__(self, tiempoCobro, T):
        ...
        self.T = T  # Tiempo constante para el cobro
    ...
    def setProximo(self, Vehiculo):
        if self.Actual:
            ...
            return EventoCobro(Vehiculo.tiempoLlegada + Vehiculo.tiempoEstacionado + self.tiempoCobro, self.T)
...
class EventoCobro:
    ...
    def __init__(self, tiempo, T):
        self.tiempo = tiempo
        self.T = T

    def simular(self):
        if self.__class__.sectorCobro.Actual:
            C = len(self.__class__.sectorCobro.vector_espera)
            if self.__class__.sectorCobro.Actual.tipo == 3:  # Grande
                D = 180
            else:  # Pequeño o Utilitario
                D = 130
            
            # Calculando el tiempo de demora utilizando la fórmula dD/dt = C + 2 * C * 0.2 * T + t^2
            t = self.tiempo - self.__class__.sectorCobro.Actual.tiempoLlegada
            dD_dt = C + 2 * C * 0.2 * self.T + t ** 2
            
            # Actualizando el tiempo de cobro
            D += dD_dt
            ...
            if self.__class__.sectorCobro.buscarSiguiente(self.tiempo):
                return [EventoCobro(self.tiempo + self.sectorCobro.tiempoCobro, self.T)]
        return []
...
l1 = LlegadaVehiculo(0).simular()
l2 = FinDeEstacionamiento(60, 4).simular()
